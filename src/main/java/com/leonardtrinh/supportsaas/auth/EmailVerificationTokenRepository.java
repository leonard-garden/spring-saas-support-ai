...
public interface EmailVerificationTokenRepository extends JpaRepository<EmailVerificationToken, UUID> {
    ...
    @Query("SELECT t FROM EmailVerificationToken t WHERE t.tokenHash = :hash AND t.usedAt IS NULL AND t.expiresAt > :now")
    Optional<EmailVerificationToken> findActiveByTokenHash(@Param("hash") String hash, @Param("now") Instant now);

    @Modifying
    @Query("UPDATE EmailVerificationToken t SET t.usedAt = :now WHERE t.memberId = :memberId AND t.usedAt IS NULL")
